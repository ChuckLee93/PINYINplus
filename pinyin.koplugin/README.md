...
* **繁体书模式(v6.5 新增,对外 v2.1)**:读繁体(传统汉字)书时在菜单打开「繁体书模式」,繁体字即按等级正常注音,并完整参与词组辨音(「銀行」→ yín háng)与生词本联动。数据表在生成期由 OpenCC 词组级转换解析简繁一对多歧义(「頭髮」→ fà、「出發」→ fā),运行时与简体通道同为纯查表,速度同构;**关闭时繁体数据不参与查表,简体书行为完全不变**(繁体词组表文件也不会被加载)。
...
> ├── trad_data.lua           # 繁体字拼音表(v6.5, 自动生成, 启动即载, 仅繁体模式查询)
> ├── polyphone_trad.lua      # 繁体多音字索引(v6.5, 自动生成, 启动即载)
> ├── fix_trad.lua            # 繁体读音修正表(v6.5, 自动生成, 启动即载)
> ├── phrase_trad.lua         # 繁体键词组辨音表(v6.5, 自动生成, 惰性加载, 仅繁体模式)
> ├── config.lua             # 高级配置(调试日志/诊断信息开关, 默认均关闭)
> └── README.md
> ```
>
> 三张 v6.0 数据表由 `gen_phrase_data.py`(上级目录)从 pypinyin 生成;四张 v6.5 繁体表由
> `gen_trad_data.py` 从对应简体表经 OpenCC 生成;均已随包提供, 无需再跑。
...
| 繁体书模式: 开/关         | v6.5:读繁体书时打开;繁体字按等级注音并参与词组辨音/生词本联动(默关;关闭时简体书行为完全不变)                                   |
